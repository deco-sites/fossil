...
function NJYouTubeModal({ videoUrl, linkText, children }: Props) {
  ...
  return (
    <>
      {/* Trigger Link or Background */}
      {children
        ? (
          <div
            onClick={handleWrapperClick}
            class={videoUrl ? "cursor-pointer" : ""}
          >
            {children}
          </div>
        )
      ...
      {videoUrl && (
        ...
        >
          ...
          <button
            type="button"
            onClick={closeModal}
            class="absolute top-2 right-2 lg:top-4 lg:right-4 z-30 text-white bg-black/50 hover:bg-black/70 rounded-full w-8 h-8 flex items-center justify-center border-none cursor-pointer transition-all duration-200 outline-none"
